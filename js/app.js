...
function initApp() {
  ...
  document.getElementById('delivery-overlay-input').addEventListener('keydown', e => {
    if (e.key === 'Escape') { closeDeliveryOverlay(); return; }
    if (e.key === 'Enter') {
      const oi = document.getElementById('delivery-overlay-input');
      _syncOverlayToMain(oi);
...
document.addEventListener('mousedown', e => {
  const btn = e.target.closest('.help-btn[data-tip-key]');
  if (btn) showPanelTip(btn, _PANEL_TIPS[btn.dataset.tipKey] || '');
});
document.addEventListener('mouseup', hidePanelTip);
document.addEventListener('touchstart', e => {
  const btn = e.target.closest('.help-btn[data-tip-key]');
  if (btn) showPanelTip(btn, _PANEL_TIPS[btn.dataset.tipKey] || '');
}, { passive: true });
document.addEventListener('touchend', hidePanelTip, { passive: true });
document.addEventListener('touchcancel', hidePanelTip, { passive: true });
...
function toggleSidebar() {
  ...
  if (isHidden) {
    ...
    if (mapBar) mapBar.classList.remove('visible');
    btn.style.display = '';
    // Focuser la dernière adresse ajoutée après l'animation de la sidebar
    setTimeout(() => { if (typeof _focusLastAdded === 'function') _focusLastAdded(); }, 320);
  }
}
